// Calls functions at the beginning
winStreakBar();
makeWinningNumbers();
makeTicket();
assignEvents();
...
// Shows bottom bar of info
function showInfo() {
    document.getElementById("info").style.bottom="0px"
}

// Hides bottom bar of info
function hideInfo() {
...
function makeTicket() {
    ...
    for(let i=0;i<=9;i++) {
        ...
        let numberToAdd=Math.floor(Math.random() * 50) + 1;
        ...
        let cellAmount=Math.floor(Math.random() * 5) + 1;
        ...
        while(cellNumbers.includes(numberToAdd)) {
            numberToAdd=Math.floor(Math.random() * 50) + 1;
        }
...
function makeWinningNumbers() {
    ...
    for(let i=0;i<4;i++) {
        ...
        winningNum.textContent=Math.floor(Math.random() * 50) + 1;
